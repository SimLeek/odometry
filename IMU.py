...
from serial_util import get_numbers_from_text
...
class PureIMUProtocol(serial.threaded.Packetizer):
    ...
    def handle_packet(self, packet):
        pkt_data = get_numbers_from_text(packet)
        if len(pkt_data) >= 9:
...
def imu_7d_pnum_acc_loop(confidence=0.99, bits=16):
    """
    Convert IMU info to to 7D pnum info

    :param confidence: Percent confidence in IMU reading, 0-100 -> 0-1
    :param bits: Number of bits needed to represent IMU info. Should approximately match IMU specifications.
    :return: A 7-dimensional probabilistic integer that should be usable with neural networks
    """
    # Note: The number of bits outputted will be bits*2^7
    for q, acc in imu_loop():
        qacc = [qu for qu in q] + [a for a in acc]
        a = PInt(*qacc, bits=bits, confidence=confidence)
        yield a
...
def imu_7d_pnum_pose_loop(confidence=0.95, bits=16, start_pos=(0, 0, 0)):
    """
    Convert IMU info to to 7D pnum info

    :param start_pos: start position in global x,y,z coordinates, orientation aligned to north-east-up
    :param confidence: Percent confidence in IMU reading, 0-100 -> 0-1
    :param bits: Number of bits needed to represent IMU info. Should approximately match IMU specifications.
    :return: A 7-dimensional probabilistic integer that should be usable with neural networks
    """
    ...
    global_pos = np.asarray(start_pos)
    t0 = time.time()
    for q, acc in imu_loop():
        t1 = time.time()
        dt = t1 - t0
        t0 = t1
        global_acc = q.conjugate() * acc * q
        global_vel += dt * global_acc
        global_pos += dt * global_vel
        qpos = [qu for qu in q] + [p for p in global_pos]
...
class IMUPoseProbabilistic(object):
    def __init__(self, confidence=0.95, bits=16, start_pos=(0, 0, 0)):
        ...
        self.global_pos = np.asarray(start_pos)
        self.t0 = time.time()
        self.confidence = confidence
        self.bits = bits

        self.imu_loop = imu_loop()

    def __iter__(self):
        for q, acc in self.imu_loop:

            t1 = time.time()
            dt = t1 - t0
            t0 = t1
            global_acc = q.conjugate() * acc * q
            self.global_vel += dt * global_acc
            self.global_pos += dt * self.global_vel
            qpos = [qu for qu in q] + [p for p in self.global_pos]
